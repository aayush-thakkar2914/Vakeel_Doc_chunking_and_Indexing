...
def chunk_document(text, filename, max_chunk_size=150):
    ...
    Break the following document into meaningful, coherent chunks (max {max_chunk_size} words each).
    
    Important guidelines:
    - Maintain context and meaning within each chunk
    - Break at natural boundaries (paragraphs, sentences, sections)
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are an expert document chunking assistant. Always return valid JSON."},
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Chunk documents and index them in OpenSearch")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        print(" Document Chunking and Indexing Tool")
        print("\nUsage examples:")
        print("  python script.py document.txt")
        print("  python script.py /path/to/documents/")
        print("  python script.py document.txt --chunk-size 200")
        print("  python script.py document.txt --index my_index")
